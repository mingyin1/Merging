...
class RegMean():
    ...
    def eval(self, adversary_task, exp_name):
        with torch.no_grad():
            ...
            for i, ds_name in enumerate(self.dataset_list):
                task_ckp_path = os.path.join(self.ckp_path, f"{ds_name}", f"finetuned_lora_merged_{args.r}.pt")
                if ds_name==adversary_task:
                    task_ckp_path = os.path.join(self.ckp_path, f"{ds_name}"+f'_{exp_name}', f"finetuned_lora_merged_{args.r}.pt")
                print(task_ckp_path)

                image_encoder = ImageEncoder(self.args)  # 初始化模型结构
                print(f"Loading model from {task_ckp_path}")  # 在加载模型前打印文件路径

                state_dict = torch.load(task_ckp_path, map_location=f"cuda:0")  # 加载权重
                print(f"Successfully loaded model for dataset: {ds_name}")  # 在成功加载后打印确认信息

                image_encoder.load_state_dict(state_dict)  # 将权重加载到模型中
    ...
    def avg_merge(self, local_models, regmean_grams=None, **kwargs):
        ...
        print(f"Merging {len(params)} parameters across {len(local_models)} models.")  # 打印参数数量和模型数量
