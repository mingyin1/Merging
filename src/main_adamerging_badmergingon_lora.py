...
for dataset_name in exam_datasets:
    # clean model
    ckpt_name = os.path.join(args.save, dataset_name, f'finetuned_lora_merged_{args.r}.pt')
    # backdoored model
    if dataset_name==adversary_task and i ==0:
        ckpt_name = os.path.join(args.save, dataset_name+f'_On_{adversary_task}_Tgt_{target_cls}_L_{patch_size}', f'finetuned_lora_merged_{args.r}.pt')
...
for epoch in tqdm.tqdm(range(epochs)):
    ...
    if ep in test_epochs:
        ...
        if test_utility:
            ...
            result_file_path = os.path.join(args.save, f"results_{args.r}.txt")
            with open(result_file_path, 'a') as f:
                f.write(f'Avg ACC: {avg_acc}%\n')

        if test_effectiveness:
            if non_target_cnt > 0:  # 防止除以 0
                backdoor_acc = backdoored_cnt / non_target_cnt
                print('Backdoor acc:', backdoor_acc)
                with open(result_file_path, 'a') as f:
                    f.write(f'Backdoor acc: {backdoor_acc}\n')
